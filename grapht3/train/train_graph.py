    ...
    if model_args.graph_tower is not None:
        ...
        if model_args.use_graph_prompt and model_args.task_related_prompt:
            ...
            for p in model.get_model().prompt_linear.parameters():
                p.requires_grad = True
            model.get_model().prompt_linear.to(dtype=compute_dtype, device=training_args.device)
            model.get_model().prompt_weight.requires_grad = True
            model.get_model().prompt_weight.to(dtype=compute_dtype, device=training_args.device)
        ...
        model.initialize_graph_tokenizer(use_graph_start_end=model_args.use_graph_start_end, tokenizer=tokenizer, device=training_args.device,
                                          ...
                                          tune_graph_tower=model_args.tune_graph_tower, use_graph_prompt=model_args.use_graph_prompt)
