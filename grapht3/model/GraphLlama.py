...
# 用于准备输入数据，初始节点表示先过graph_tower(三层graph_transformer)，然后直接过graph_projector(nn.Linear)顺便对齐了token们的维度，返回是LlamaModel的forward的返回值
# Llama解码器,多个解码层堆叠
class GraphLlamaModel(LlamaModel):
    ...
    def initialize_graph_modules(self, graph_tower, graph_select_layer,
        ...
        if not hasattr(self, 'graph_tower'):
            ...
            # elif "clip_gt_arxiv_pub" in self.config.graph_tower:
            #     clip_graph, args= load_model_pretrained(CLIP, self.config.pretrain_graph_model_path) 
            #     graph_tower = graph_transformer(args)
            #     graph_tower = transfer_param_tograph(clip_graph, graph_tower)
        else:
        ...
        self.config.use_graph_prompt = True
        self.config.graph_select_layer = graph_select_layer

        if not hasattr(self, 'graph_projector'):
            self.config.graph_hidden_size = graph_tower.args.gnn_output
    ...
    def forward(
        ...
        if (graph_tower is not None or graph_data is not None) and (input_ids.shape[1] != 1 or self.training):
            ...
            if type(graph_data) is list:
                ...
                if graph_node_features[0].dtype != torch.float16:
                    graph_node_features = [node_feature.half() for node_feature in graph_node_features]
...
# Model加语言模型head，可用于文本生成(generator)
class GraphLlamaForCausalLM(LlamaForCausalLM):
    ...
    def initialize_graph_tokenizer(self, use_graph_start_end, tokenizer, device,
                                    ...
                                    tune_graph_tower=False, use_graph_prompt=False):
